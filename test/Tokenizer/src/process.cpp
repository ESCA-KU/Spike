...
struct llm_tokenizer_spm {
    ...
    void tokenize(const std::string & text, std::vector<llama_vocab::id> & output) {
        ...
        /*
        clock_t tokenize_end = clock();
        printf("number of tokens: %ld\n", output.size());
        printf("Priority queue max size: %d\n", queue_maxsize);
        printf("max token length: %d\n", max_token_len);
        

        printf("tokenizer time: %lf\n", (double)(tokenize_end - tokenize_start) / CLOCKS_PER_SEC);
        printf("hash time: %lf\n", hash_time / CLOCKS_PER_SEC);
        printf("pq push time: %lf\n", push_time / CLOCKS_PER_SEC);
        */
        /*
        ...
        */
/*
        std::ofstream outfile("/gem5/mycode/tokenize/vector-ksc-result.txt", std::ios_base::app);
        if (!outfile.is_open()) {
            std::cerr << "파일을 열지 못했습니다.\n";
        }
        outfile << pq_time << ' ' << hash_time << "\n";
        outfile.close();
  */
        //std::cout << pq_time << ' ' << hash_time << "\n";
...
//#endif
        ...
        //clock_t pq_start = clock();
        my_work_queue.push(bigram);
        //clock_t pq_end = clock();
        
        //pq_time += pq_end - pq_start;
