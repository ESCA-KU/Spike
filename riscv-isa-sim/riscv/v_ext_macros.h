...
#define VSHFT_LD(index, elt_width, max_value, is_mask_ldst) \
  ...
  for(reg_t i = 0; i < vl; ++i) { \
    ...
    for(reg_t fn = 0; fn < nf; ++fn) { \
      elt_width##_t val; \
      if((index >> (i + 1)) < 1) val = max_value; \
      else val = MMU.load<elt_width##_t>( \
        baseAddr + (index >> (i + 1)) * sizeof(elt_width##_t)); \
      P.VU.elt<elt_width##_t>(vd + fn * emul, vreg_inx, true) = val; \
    } \
  } \
  P.VU.vstart->write(0);

#define VSHFT_ST(index, elt_width, is_mask_ldst) \
  const reg_t nf = insn.v_nf() + 1; \
  const reg_t vl = is_mask_ldst ? ((P.VU.vl->read() + 7) / 8) : P.VU.vl->read(); \
  const reg_t baseAddr = RS1; \
  ...
  for(reg_t i = 0; i < vl; ++i) { \
    ...
    for(reg_t fn = 0; fn < nf; ++fn) { \
      ...
      MMU.store<elt_width##_t>(\
        baseAddr + (index >> i) * sizeof(elt_width##_t), val); \
    } \
  } \
  P.VU.vstart->write(0);


#define VCAST_LD(elt_width, is_mask_ldst) \
const reg_t nf = insn.v_nf() + 1; \
VI_CHECK_LOAD(elt_width, is_mask_ldst); \
const reg_t vl = is_mask_ldst ? ((P.VU.vl->read() + 7) / 8) : P.VU.vl->read(); \
const reg_t baseAddr = RS1; \
...
for (reg_t i = 0; i < vl; ++i) { \
  ...
  for (reg_t fn = 0; fn < nf; ++fn) { \
    uint8_t val = MMU.load<uint8_t>( \
      baseAddr + i * sizeof(uint8_t)); \
    P.VU.elt<elt_width##_t>(vd + fn * emul, vreg_inx, true) = (uint32_t)val; \
  } \
} \
P.VU.vstart->write(0);
...
#define VSTRCMP() \
  ...
  reg_t rs2_num = insn.rs2(); \
  uint64_t res = 1; \
  uint64_t &vdi = P.VU.elt<uint64_t>(insn.rd(), 0, true); \
  for (reg_t i = P.VU.vstart->read(); i < vl; ++i) { \
    VI_LOOP_ELEMENT_SKIP(); \
    uint64_t mmask = UINT64_C(1) << mpos; \
    VV_PARAMS(e8) ;\
